...
!!! tip "🧭 이 시간을 이렇게 읽으세요 (비개발자용)"
    - **한 줄 핵심**: 텍스트를 **숫자 좌표(벡터)** 로 바꿔 "비슷한 의미" 를 빠르게 찾는 원리를 이해하고, 그 좌표를 디스크에 저장(영속화)합니다.
    - **꼭 이해**: "의미가 비슷한 문장은 좌표가 가깝다" 는 직관 + ChromaDB 의 영속화 = "노트북을 재시작해도 이전에 저장한 내용이 살아남기" 라는 점.
    - **지금은 몰라도 OK**: 코사인 유사도의 수식, 1536차원 벡터의 내부 의미. 화살표 두 개가 같은 방향이면 1, 직각이면 0 — 이 정도면 충분합니다.
    - **막히면**: [용어 사전 — 임베딩·코사인 유사도·벡터 DB](../appendix/glossary.md#c-ai-llm) 항목.
