...
!!! tip "🧭 이 시간을 이렇게 읽으세요 (비개발자용)"
    - **한 줄 핵심**: 4일 후 발표할 **본인 도메인 SQL 에이전트** 의 첫 번째 과제(제안서) 를 받고, 무엇으로 평가되는지 확인하는 시간입니다.
    - **꼭 이해**: 4단계 마일스톤(**제안서 → DB 배포 → 에이전트 v1 → 최종 발표**) 의 흐름과 본인 도메인 후보 1~2개.
    - **지금은 몰라도 OK**: 평가 루브릭의 세부 가중치 숫자, "Hard 난이도 질문" 의 형태 — 제안서를 쓰며 자연스럽게 익숙해집니다.
    - **막히면**: [프로젝트 브리프 부록](../appendix/project-brief.md) 에 양식·예시·FAQ 가 정리되어 있고, 모르는 단어는 [용어 사전](../appendix/glossary.md) 으로.
