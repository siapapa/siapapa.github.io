...
!!! tip "팁"
    본인이 잘 아는 도메인을 선택하세요. 업무 지식이 풍부할수록 스키마 설계와 질문 작성이 수월합니다.
...
2. **아키텍처** — LangGraph 상태 다이어그램, 데이터 흐름, 사용 도구 선택 이유
3. **결과 및 회고** — Ragas 메트릭 (개선 전 vs 후), 성공/실패 사례, 배운 점
...
```
잘못된 예:  p_id, v_dt, sal, dept_cd
올바른 예:  patient_id, visit_date, salary, department_code
```
...
!!! note "핵심 정리"
    - **시작이 반이다.** 1H에서 봤던 완성 에이전트를 떠올리며, 4일 후에는 본인만의 에이전트를 만들어 발표하게 됩니다
    - 도메인 선정부터 시작하세요 — 본인이 잘 아는 분야가 최선입니다
    - 스키마의 `COMMENT ON`이 에이전트 성능의 핵심입니다
