...
!!! tip "🧭 이 시간을 이렇게 읽으세요 (비개발자용)"
    - **한 줄 핵심**: 지금까지 배운 모든 도구를 모아 **본인 프로젝트의 SQL 에이전트 v1** 을 가동합니다.
    - **꼭 이해**: 1H 데모 에이전트와 같은 5단계(질문 → SQL 생성 → 실행 → 검증 → 답변) 를 본인 도메인 스키마로 베끼면 됩니다.
    - **지금은 몰라도 OK**: 모든 노드를 처음부터 새로 짜기. **`00_demo_agent.ipynb` 를 베껴 시작** 하세요.
...
!!! warning "🆘 비개발자를 위한 회복 가이드 — 여기까지 어렵다면"
    이 시간은 지금까지 배운 도구를 **모두 모아 본인 프로젝트 에이전트** 를 만드는 시간입니다.

    1. 코드 한 줄씩 외우려 하지 마세요. **"질문 → SQL 생성 → 실행 → 검증 → 답변"** 다섯 단어가 전부입니다.
    2. 막히면 **강사가 시연한 `00_demo_agent.ipynb` 의 노드 함수들을 본인 도메인으로 베껴 시작** 하세요. 처음부터 새로 짜지 마세요.
    3. v1 의 목표는 "10개 질문 중 5개라도 정답" 이면 충분합니다. Day 4 LangSmith 트레이싱으로 어디서 틀렸는지 확인하고 고쳐 나갑니다.

    → 더 막힌다면 [비개발자 학습 가이드](../beginners-guide.md) 의 "막혔을 때" 섹션 참조.
