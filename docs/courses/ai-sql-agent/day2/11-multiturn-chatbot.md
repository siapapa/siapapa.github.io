...
!!! tip "🧭 이 시간을 이렇게 읽으세요 (비개발자용)"
    - **한 줄 핵심**: 한 번 묻고 끝이 아니라 **"이전 대화를 기억하고 이어가는 챗봇"** 의 골격을 설계합니다.
    - **꼭 이해**: 멀티턴 = "메모지에 대화 이력을 누적하며 다음 SQL 을 만들기". 가드레일(금칙어·범위 제한) 의 필요성.
    - **지금은 몰라도 OK**: 가드레일을 정규식으로 완벽하게 짜는 것 — 사전 준비 페이지의 read-only 세션이 진짜 방어선입니다.
