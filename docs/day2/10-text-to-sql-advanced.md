...
!!! tip "🧭 이 시간을 이렇게 읽으세요 (비개발자용)"
    - **한 줄 핵심**: 어제 맛본 Text-to-SQL 을 더 깊이 들여다보며, **AI 에게 더 좋은 정보(스키마·예시)를 주면 정답률이 어떻게 올라가는지** 체감합니다.
    - **꼭 이해**: AI 가 보는 프롬프트는 "스키마 + 예시 + 질문" 의 묶음이라는 점. 예시(Few-shot)를 넣으면 정확도가 올라갑니다.
    - **지금은 몰라도 OK**: 프롬프트 추적 로그(`verbose=True`)의 모든 줄을 한 글자씩 따라 읽기. 흐름만 잡으세요.
